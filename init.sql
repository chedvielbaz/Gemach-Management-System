-- init.sql — סכמת מסד הנתונים לפרויקט מרכז הגמחים (SQL Server)
-- הריצו בסקריפט על המופע שלכם לפני dotnet run.
-- אין כאן נתוני משתמש אמיתיים; להרשמה והוספת גמ״ח משתמשים בממשק האתר.
...
-- קטגוריות בסיסיות בלבד (ללא לקוחות / גמחים אמיתיים)
